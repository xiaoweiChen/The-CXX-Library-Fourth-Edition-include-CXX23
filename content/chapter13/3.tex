字符串拥有的元素数量(str.size())通常小于保留空间的元素数量:str.capacity()。因此，若向字符串中添加元素，将不会自动分配新的内存。std::max\_size()返回一个字符串最大可以包含多少个元素。对于这三个成员函数，下列关系成立:str.size() <= str.capacity() <= str.max\_size()。
