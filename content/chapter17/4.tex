...
std::vector的特化具有成员函数parse和format。parse实际上创建了一个formatString，应用于std::vector(1和2)的每个元素。解析上下文ctx(3)包含冒号(:)和右花括号(\})之间的字符。该函数返回一个指向右花括号(\})的迭代器。成员函数格式的工作更有趣。format上下文返回输出迭代器。因为使用了输出迭代器和\href{https://en.cppreference.com/w/cpp/utility/format/format_to}{std::format\_to}函数，std::vector的元素可以很好地显示出来。
